...
function filtered() { return lessons.filter(x=>(!els.level.value||x.level===els.level.value)&&(!els.sub.value||x.subLevel===els.sub.value)&&(!els.unit.value||String(x.unit)===els.unit.value)); }
function resetActivity() { els.activity.value=""; $("activity").hidden=true; $("tf-form").reset(); $("open-form").reset(); }

els.level.addEventListener("change",()=>{setOptions(els.sub,unique(lessons.filter(x=>x.level===els.level.value).map(x=>x.subLevel)),"Select sub-level");setOptions(els.unit,[],"Select unit");setOptions(els.activity,[],"Select activity");resetActivity();});
els.sub.addEventListener("change",()=>{setOptions(els.unit,unique(filtered().map(x=>x.unit)),"Select unit");setOptions(els.activity,[],"Select activity");resetActivity();});
els.unit.addEventListener("change",()=>{const items=filtered();els.activity.replaceChildren(option("","Select activity"),...items.map(x=>option(x.id,x.title)));els.activity.disabled=!items.length;resetActivity();});
...
function renderLesson(lesson) {
  ...
  $("activity-title").textContent=lesson.title; $("activity-path").textContent=`${lesson.level} • ${lesson.subLevel} • Unit ${lesson.unit}`; $("listening-level").textContent=`${lesson.listeningLevel} listening`;
